...
begin
  if FTolleranceAdjust > tolerance then
    tolerance := FTolleranceAdjust;
end;


procedure TGPointPresenter.Assign( Source : TPersistent );
begin
  inherited Assign( Source );

  if Source is TGPointPresenter then
  begin
    PointType := TGPointPresenter( Source ).PointType;
    PointUnit := TGPointPresenter( Source ).PointUnit;
    PointSize := TGPointPresenter( Source ).PointSize;
    SymbolIndex := TGPointPresenter( Source ).SymbolIndex;
    ImageName := TGPointPresenter( Source ).ImageName;
    FiMaxPointSize := TGPointPresenter( Source ).MaxPointSize;
    PointPen.Assign( TGPointPresenter( Source ).PointPen );
    PointBrush.Assign( TGPointPresenter( Source ).PointBrush );
    PointFont.Assign( TGPointPresenter( Source ).PointFont );
  end;
end;
...
begin
  ...
  FTolleranceAdjust := TGlobe5(ParentGlobe).GlobeUnitsFrom( PointSize + 1, PointUnit ) div 2;

  if MaxPointSize <> 0 then
    FTolleranceAdjust := MinVal( FTolleranceAdjust, Round( MaxPointSize / ParentGlobe.Projector.Scalefactor ));
end;
...
begin
  with mapObj.Centroid do
    Result := MER_ContainsLL( MER( iLongX - FTolleranceAdjust, iLatY - FTolleranceAdjust, FTolleranceAdjust * 2, FTolleranceAdjust * 2 ), ptLL);
end;
...
begin
  ...
  case PointType of
    ...
    begin
      ...
      begin
        Renderer.AlphaBlend := AlphaBlend;
  ...
  case PointType of
    ppFontSymbol : { display a font symbol }
      begin
        ...
        Renderer.DrawText( X, Y, sTmp, taCenter, 0 );
      end;
    ...
    ppDot : { display a single pixel }
      begin
        Renderer.DrawPixel(X,Y, PointPen.Color, AlphaBlend );
      end;
...
procedure TGPolyPresenter.Assign( Source : TPersistent );
begin
  inherited Assign( Source );

  if Source.InheritsFrom( TGPolyPresenter ) then
  begin
    PolyPen.Assign( TGPolyPresenter( Source ).PolyPen );
    PolyBrush.Assign( TGPolyPresenter( Source ).PolyBrush );
    PolyRenderMode := TGPolyPresenter( Source ).PolyRenderMode;
  end;
end;
