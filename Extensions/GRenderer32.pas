...
begin
  if ( closed and (( Brush.Style = gbsSolid ) or ( Brush.Style = gbsClear))) or ( not closed and ( Pen.Style = gpsSolid ) and (Pen.Width = 1 )) then
  begin
    polygon := TPolygon32.Create;
    polygon.Closed := closed;

//  You can uncomment the lines below if using Graphics32 7.xx or above
//    polygon.Antialiased := Antialiased;
//    if Antialiased then
//      polygon.AntialiasMode := am4times; // use low/fast level

    try
      // copy over the points
      for jdx := 0 to High( Points ) do
        if Length( Points[jdx] ) > ( 1 + Ord(closed)) then
        begin
      ...
      if Length( polygon.Points ) = 0 then
        Exit;

      if not closed or ( Brush.Style = gbsClear ) then
      begin
...
begin
  ...
  // draw the scanline left to right
...
//        FBackBuffer.Bits[x1] := ToColor(image.Pixel[ Trunc(u1), Trunc(v1)]);
    ...
    begin
      if ( u1 < srcW ) and ( v1 < srcH ) and ( u1 >= 0.0 ) and ( v1 >= 0.0 ) then
      begin
        srcColor := image.BilinearPixel( u1, v1 );
